<template>
    <div class="main-content">
        ...
        <!-- Modal Add Payment by selcetion-->
        <validation-observer ref="Add_payment_by_selection">
            <b-modal
                hide-footer
                size="lg"
                id="Add_Payment_by_selection"
                :title="$t('Create Payment By Selection')"
            >
                <b-form @submit.prevent="Submit_Payment_By_Selection">
                    <b-row>
                        <!-- date -->
                        <b-col lg="6" md="12" sm="12">
                            <validation-provider
...
export default {
    ...
    methods: {
        ...
        //------ Validate Form Submit_Payment_By_Selection
        Submit_Payment_By_Selection() {
            this.paymentProcessing = true;
            NProgress.start();
            NProgress.set(0.1);
            axios
                .post("payment/purchase/selection", {
                    purchases: this.selectedIds,
                ...
                .then((response) => {
                    ...
                    this.$bvModal.hide("Add_Payment_by_selection");
                    this.makeToast(
                        "success",
        ...
        pay_by_selected() {
            ...
            this.results = this.purchases.filter(
                (item) =>
                    item.payment_status !== "paid" &&
                    this.selectedIds.indexOf(item.id) !== -1
            );
            let sum = 0;
            for (let i = 0; i < this.results.length; i++) {
                sum = sum + parseFloat(this.results[i].due);
            }
            this.facture.date = new Date().toISOString().slice(0, 10);
            this.Number_Order_Payment();
            this.facture.montant = sum;
            this.facture.Reglement = "Cash";
            this.facture.received_amount = sum;
            this.due = parseFloat(sum);
            setTimeout(() => {
                // Complete the animation of the  progress bar.
                NProgress.done();
                this.$bvModal.show("Add_Payment_by_selection");
            }, 500);
        },
