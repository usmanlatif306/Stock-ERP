    ...
    <div class="main-content">
        ...
        >
            <b-row>
                ...
                <b-col md="3">
                    <b-button variant="primary" @click="ExpiryDateFunction">{{
                        $t("Save")
                    }}</b-button>
                </b-col>
            </b-row>
        ...
        >
            <b-row>
                <b-col md="9">
                    <b-form-input
                        :placeholder="$t('Enter Batch No')"
                        v-model="batchNum"
                    ></b-form-input>
                </b-col>
                <b-col md="3">
                    <b-button variant="primary" @click="BatchNoFunction">{{
                        $t("Save")
                    }}</b-button>
                </b-col>
            </b-row>
...
export default {
    ...
    methods: {
        ...
        // Show Expiry Date model
        Modal_Expiry_Date(id) {
            this.expiryId = id;
            this.$bvModal.show("update_expiry_date");
        },
        // Show Batch No model
        Modal_Batch_No(id) {
            this.batchId = id;
        ...
        ExpiryDateFunction() {
            for (var i = 0; i < this.details.length; i++) {
                if (this.details[i].detail_id == this.expiryId) {
                    this.details[i].expiry_date = this.expiryDate;
                }
            }
            this.$forceUpdate();
            this.$bvModal.hide("update_expiry_date");
        },
        ...
        BatchNoFunction() {
            for (var i = 0; i < this.details.length; i++) {
                if (this.details[i].detail_id == this.batchId) {
                    this.details[i].batch_no = this.batchNum;
                }
            }
            this.$forceUpdate();
