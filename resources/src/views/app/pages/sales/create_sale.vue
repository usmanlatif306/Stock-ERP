                ...
                <b-row>
                    ...
                    <b-col md="12">
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="(item, idx) in batches" :key="idx">
...
export default {
    ...
    data() {
        return {
            ...
            detail_id: "",
            batchNum: "",
            // fields: ['expiry_date', 'batch_number', 'quantity'],
        };
    },
    ...
    methods: {
        ...
        //------ Show Modal Update Batches
        Modal_Update_Batch(detail) {
            this.batches = detail.purchase_detail;
            this.detail_id = detail.detail_id;
            this.batchNum = detail.batch_no;
            this.$bvModal.show("update_batch_number");
        },
        // BatchNoFunction() {
        //     this.$bvModal.hide("update_batch_number");
        // },
        ...
        SearchProduct(result) {
            ...
            if (
                ...
                this.product.purchase_detail = result.product_purchase;
        ...
        Selected_Client(value) {
            ...
            this.sale.warehouse_id = warehouse.id;
            this.search_input = "";
            this.product_filter = [];
            this.Get_Products_By_Warehouse(warehouse.id);
        },
        //---------------------- Event Select Batch ------------------------------\\
        Selected_Batch(product) {
            for (var i = 0; i < this.details.length; i++) {
                if (this.details[i].detail_id === this.detail_id) {
                    this.details[i].batch_no = product.label;
                    this.details[i].expiry_date = product.expiry_date;
                    // this.details[i].purchase_id = product.value;
                    this.$forceUpdate();
                }
            }
            this.$bvModal.hide("update_batch_number");
            // let already = this.sale.product_purchases.find(
            //     (item) =>
            ...
            //     let filterArr = this.sale.product_purchases.filter(
            //         (item) =>
            //             item.product_id !== product.product_id &&
            //             item.product_variant_id !== product.product_variant_id
            //     );
            //     filterArr.push(product);
            //     this.sale.product_purchases = filterArr;
            // } else {
            //     // product not in list
            //     this.sale.product_purchases.push(product);
            // }
        },
                ...
                if (this.payment.Reglement == "credit card") {
                    ...
                    axios
                        .post("sales", {
                            ...
                            GrandTotal: this.GrandTotal,
                            details: this.details,
                            product_purchase: this.sale.product_purchases,
                            payment: this.payment,
                            amount: parseFloat(this.payment.amount).toFixed(2),
                            received_amount: parseFloat(
                                this.payment.received_amount
