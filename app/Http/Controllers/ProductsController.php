...
class ProductsController extends BaseController
{
    ...
    public function Products_by_Warehouse(request $request, $id)
    {
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            $productPurchase = PurchaseDetail::where('product_id', $item['id'])->where('product_variant_id', $item['product_variant_id'])->where('expiry_date', '>', now())->get(['id', 'expiry_date', 'batch_no', 'product_id', 'product_variant_id', 'quantity'])->map(function ($productPurchase) {
                return [
                    'label' => $productPurchase->batch_no,
                    'value' => $productPurchase->id,
                    'expiry_date' => $productPurchase->expiry_date,
                    'quantity' => $productPurchase->quantity,
                    // 'product_id' => $productPurchase->product_id,
                    // 'product_variant_id' => $productPurchase->product_variant_id,
                ];
            });

            $item['product_purchase'] = $productPurchase;
    ...
    //------------ Get products By Warehouse With No Varient -----------------\\

    public function Products_by_Warehouse_No_Varient(request $request, $id)
    {
        $data = [];
        $product_warehouse_data = product_warehouse::with('warehouse', 'product')
            ->where('warehouse_id', $id)
            ->where('deleted_at', '=', null)
            ->where(function ($query) use ($request) {
                if ($request->stock == '1') {
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if (!in_array($product_warehouse['product']->name, $name)) {
                $item['value'] = $product_warehouse->product_id;
                $item['label'] = $product_warehouse['product']->name;
                $data[] = $item;
    ...
    public function Products_by_Location(request $request, $id)
    {
        $data = [];
        $product_location_data = ProductLocation::with('location', 'product', 'productVariant')
        ...
        foreach ($product_location_data as $product_warehouse) {
