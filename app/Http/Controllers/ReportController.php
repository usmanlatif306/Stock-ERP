...
class ReportController extends BaseController
{
    ...
    public function Adjustments_Warehouse(request $request)
    {
        ...
        $adjustments = Adjustment::where('deleted_at', '=', null)->with(['details', 'warehouse'])
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
        ...
        foreach ($adjustments as $adjustment) {
            foreach ($adjustment->details as $detail) {
                ...
                $item['warehouse'] = $adjustment->warehouse->name;
                $item['product'] = $detail->product->name;
                $item['quantity'] = $detail->quantity;
                $item['status'] = $detail->type == 'sub' ? 'substraction' : 'addition';
    ...
    public function Transfers_Warehouse(request $request)
    {
        ...
        $transfers = Transfer::where('deleted_at', '=', null)->with(['details', 'from_warehouse', 'to_warehouse'])
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
            ...
            ->orWhere(function ($query) use ($request) {
                return $query->when($request->filled('warehouse_id'), function ($query) use ($request) {
                    return $query->where('to_warehouse_id', $request->warehouse_id);
                });
            });
        ...
        foreach ($transfers as $transfer) {
            foreach ($transfer->details as $detail) {
                ...
                $item['from'] = $transfer->from_warehouse->name;
                $item['to'] = $transfer->to_warehouse->name;
                $item['product'] = $detail->product->name;
                $item['quantity'] = $detail->quantity;
                $item['total'] = $detail->total;
                $item['status'] = $transfer->statut;
