...
class PurchaseOrderController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $Quotations = PurchaseOrder::with('client', 'location')
        ...
        foreach ($Quotations as $Quotation) {
            ...
            $item['warehouse_name'] = $Quotation['location']->name;
        ...
        $warehouses = Location::where('deleted_at', '=', null)->get(['id', 'name']);
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            $order->location_id = $request->warehouse_id;
            ...
            PurchaseOrderDetail::insert($orderDetails);
        }, 10);
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            $role = Auth::user()->roles()->first();
            $view_records = Role::findOrFail($role->id)->inRole('record_view');
            $current_Quotation = PurchaseOrder::findOrFail($id);
            ...
            $old_quotation_details = PurchaseOrderDetail::where('purchase_order_id', $id)->get();
            ...
            foreach ($old_quotation_details as $key => $value) {
                ...
                if (!in_array($old_detail_id[$key], $new_details_id)) {
                    $QuotationDetail = PurchaseOrderDetail::findOrFail($value->id);
            ...
            foreach ($new_quotation_details as $key => $product_detail) {
                ...
                $QuoteDetail['purchase_order_id'] = $id;
                ...
                if (!in_array($product_detail['id'], $old_detail_id)) {
                    PurchaseOrderDetail::Create($QuoteDetail);
                } else {
                    PurchaseOrderDetail::where('id', $product_detail['id'])->update($QuoteDetail);
                }
            }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            $role = Auth::user()->roles()->first();
            $view_records = Role::findOrFail($role->id)->inRole('record_view');
            $Quotation = PurchaseOrder::findOrFail($id);
    ...
    public function delete_by_selection(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($selectedIds as $Quotation_id) {
                $Quotation = PurchaseOrder::findOrFail($Quotation_id);
    ...
    {
        // $this->authorizeForUser($request->user('api'), 'view', Quotation::class);
        return Excel::download(new QuotationsExport, 'Purchase_Order.xlsx');
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $role = Auth::user()->roles()->first();
        $view_records = Role::findOrFail($role->id)->inRole('record_view');
        $quotation_data = PurchaseOrder::with('details.product.unitSale')
            ->where('deleted_at', '=', null)
            ->findOrFail($id);
        ...
        $quote['warehouse'] = $quotation_data['location']->name;
    ...
    public function Quotation_pdf(Request $request, $id)
    {
        ...
        $details = array();
        $helpers = new helpers();
        $Quotation = PurchaseOrder::with('details.product.unitSale')
            ->where('deleted_at', '=', null)
            ->findOrFail($id);
        ...
        $pdf = \PDF::loadView('pdf.purchase_order_pdf', [
            'symbol' => $symbol,
            'setting' => $settings,
            'quote' => $quote,
            'details' => $details,
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($Quotation->location_id) {
            if (Location::where('id', $Quotation->location_id)
                ->where('deleted_at', '=', null)
                ->first()
            ) {
                $quote['location_id'] = $Quotation->location_id;
            } else {
                $quote['location_id'] = '';
            }
        } else {
        ...
        foreach ($Quotation['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                $item_product = ProductLocation::where('product_id', $detail->product_id)
                    ->where('product_variant_id', $detail->product_variant_id)
                    ->where('location_id', $Quotation->location_id)
                    ->where('deleted_at', '=', null)
                    ->first();
    ...
    {
        $Quotation = PurchaseOrder::where('deleted_at', '=', null)->findOrFail($request->id);
        $url = url('/api/Quote_PDF/' . $request->id);
        $receiverNumber = $Quotation['client']->phone;
        $message = "Dear" . ' ' . $Quotation['client']->name . " \n We are contacting you in regard to a Purchase Order #" . $Quotation->Ref . ' ' . $url . ' ' . "that has been created on your account. \n We look forward to conducting future business with you.";
        try {
