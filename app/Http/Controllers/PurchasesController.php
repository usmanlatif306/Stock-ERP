...
use App\Http\Resources\PurchaseLedgerResource;
...
class PurchasesController extends BaseController
{
    ...
    {
        $end = Carbon::make($request->end)->addDay(1)->format('Y-m-d');

        $ledgers = Purchase::with('facture')->whereHas('facture')->where('provider_id', $request->supplier)->whereBetween('created_at', [$request->start, $end])->where('deleted_at', '=', null)->orderBy('created_at')->get();
        $totalRows = $ledgers->count();

        return response()->json([
            'totalRows' => $totalRows,
            'ledgers' => PurchaseLedgerResource::collection($ledgers),
            // 'ledgers' => $ledgers,
        ]);
    }
    ...
    public function index(request $request)
    {
        ...
        $Purchases = Purchase::with('facture', 'provider', 'warehouse', 'details')
        ...
        foreach ($Purchases as $Purchase) {
            ...
            // $item['expiry_date'] = $Purchase->details[0]['expiry_date'];
            // $item['batch_no'] = $Purchase->details[0]['batch_no'];
    ...
    public function show(Request $request, $id)
    {
        ...
        foreach ($purchase['details'] as $detail) {
            ...
            $data['expiry_date'] = $detail->expiry_date;
            $data['batch_no'] = $detail->batch_no;
