...
class PaymentPurchasesController extends BaseController
{
    ...
    //----------- Store New Payment Purchase By Selection --------------\\

    public function paymentBySelection(Request $request)
    {
        $this->authorizeForUser($request->user('api'), 'create', PaymentPurchase::class);

        foreach ($request->purchases as $id) {
            \DB::transaction(function () use ($request, $id) {
                ...
                $total_paid = $purchase->GrandTotal;
                $due = 0.0;
